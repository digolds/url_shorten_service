...
测试数据的准备是一个巨大的挑战。这些挑战有以下几种：

1. 如何准备大量的数据，比如超过1TB的数据
2. 如何确保并发执行的测试不会修改其它测试用例的数据
3. 如何确保End to End的测试数据是完整的

## 生成测试报告

测试报告是测试团队对外的输出结果，因此非常重要！大家不仅可以通过这份测试报告来了解产品的运行状态，而且可以通过这份报告来排查问题。公司需要根据这些报告对客户发布SLAs条款，比如1000个用户使用URL Shortening Service生成短链接，其中999个用户所等待的时间将小于2ms。以下是一份关于URL Shortening Service的测试报告(红色部分是没有达标的，绿色部分则是通过的)：
